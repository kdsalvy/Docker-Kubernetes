...
* docker inspect {container name} : Lists all the network settings
...
example - docker network create --driver bridge --subnet 182.18.0.1/24 wp-mysql-network --gateway 182.18.0.1
...
Pod – Smallest component of the system. Generally have 1:1 relation with containers except in case of helper container where it becomes the part of the same pod with main container.
Replication Controller – Old technology to create replicas in order to scale our application
Replica Set – New version of replication controller
Deployment – deployment encapsulates replica-set and provides way to ensure rolling updates or rollback, pause and stop 
...
Kubernetes Service – Enables communication between applications or users outside of the environment. Enables loose coupling between micro-services in our application.
•	Node Port: Listens & maps a port on the node to the port on the pod and make it accessible to the outside world.
•	Cluster IP: Cluster
...
Kubernetes Commands:
•	To create container:
kubectl create -f k8.yml – creates and runs a pod/deployment/service defined in k8.yml
•	To expose container:
